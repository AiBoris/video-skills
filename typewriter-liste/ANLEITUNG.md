# Anleitung: Typewriter-Listen-Videos einrichten
...
Entpacke `typewriter-liste-skill.zip` per Doppelklick.
...
`typewriter-liste-skill` mit der Maus ins Terminal-Fenster.** Der Pfad wird
...
```
npx skills add /Users/deinname/Downloads/typewriter-liste-skill --global --copy --all
```
...
```
Failed to install 2
  typewriter-liste -> Eve: Eve does not support global skill installation
  typewriter-liste -> PromptScript: ...
```

**Dann ist alles in Ordnung.** Der Skill wird für rund 80 Agenten gleichzeitig
installiert. Zwei davon koennen keine globale Installation — die benutzt du
ohnehin nicht.

Entscheidend ist die Zeile weiter oben:

```
typewriter-liste (copied)
```
...
> Mach mir ein Typewriter-Listen-Video über Ausreden im Vertrieb

Er fragt nach, ob du den Text vorgeben willst oder ob er ihn schreiben soll,
legt dir die Liste zur Freigabe vor und rendert dann das fertige MP4.
...
Normal, siehe Schritt 3. Solange `typewriter-liste (copied)` erscheint, hat es
geklappt.
...
die Datei `SKILL.md` direkt drin liegt — also auf `typewriter-liste-skill`,
...
Dort muss `typewriter-liste` auftauchen.
...
- Nur **Großbuchstaben** — Kleinbuchstaben brechen die Schreibmaschinen-Optik
- Maximal **44 Zeichen pro Zeile**
- Maximal **9 Zeilen** inklusive Überschrift
- **Kein Ton**, 16:9, etwa 10 bis 20 Sekunden

Umlaute (Ä Ö Ü) funktionieren ohne Umschreiben.
